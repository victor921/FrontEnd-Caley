...
export default {
  ...
  methods: {
    ...
    async fetchCompanyData() {
      ...
      this.loading = true;
      try {
        const response = await axios.get(`/api/get_file_content?path=/caley-operations-dev/Static Files/company_metadata.json&code=${process.env.VUE_APP_FUNCTION_KEY}`);
    ...
    async fetchLastEditedLog() {
      try {
        const response = await axios.post(`/api/query_db?code=${process.env.VUE_APP_FUNCTION_KEY}`, {
    ...
    async saveChanges() {
      ...
      try {
        await axios.post(`/api/create_file?code=${process.env.VUE_APP_FUNCTION_KEY}`, {
    ...
    async updateLog() {
      ...
      await axios.post(`/api/update_sql?code=${process.env.VUE_APP_FUNCTION_KEY}`, { table: "configuration_logs", data: logData });
    },
